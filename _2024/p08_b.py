...
def run(s: str) -> int:
    ...
    for frequency, locations in frequency_to_locations.items():
        for loc_1, loc_2 in combinations(locations, 2):
            anti_node_locations.add(loc_1)
            anti_node_locations.add(loc_2)

            diff = loc_2 - loc_1

            loc_before = loc_1 - diff
            while check_is_within_bounds(loc_before, num_rows, num_columns):
                anti_node_locations.add(loc_before)
                loc_before -= diff

            loc_after = loc_2 + diff
            while check_is_within_bounds(loc_after, num_rows, num_columns):
                anti_node_locations.add(loc_after)
                loc_after += diff
