...
def run(s: str) -> int:
    ...
    wire_to_value = get_output_wires(lines, {})
    a_value = wire_to_value["a"]
    new_wire_to_value = get_output_wires(lines, {"b": a_value})

    return new_wire_to_value["a"]


def get_output_wires(lines: list[str], known_values: dict[str, int]) -> dict[str, int]:
    lines_to_process = deepcopy(lines)

    wire_to_value = deepcopy(known_values)
...
def process_line(line: str, wire_to_value: dict[str, int]) -> bool:
    ...
    if wire in wire_to_value:
        # we do not want to override a value we already know
        return True
